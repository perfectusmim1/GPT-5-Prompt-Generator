...
export default function ResultCard({ content }: Props) {
  ...
  return (
    <motion.div
      ...
      className="relative w-full max-w-3xl overflow-hidden rounded-2xl border border-white/10 bg-white/60 dark:bg-white/5 backdrop-blur-xl p-0 shadow-[0_10px_40px_rgba(0,0,0,0.15)] cursor-pointer text-left"
    >
      {/* Hover ipucu */}
      <div className="flex items-center justify-between px-5 py-3 select-none">
        <h2 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Generated Prompt</h2>
        <motion.span
          className="text-xs text-indigo-300/90 dark:text-indigo-300/70"
          initial={{ opacity: 1 }}
      ...
      <motion.pre
        className="scroll-area px-5 pb-5 whitespace-pre-wrap leading-relaxed text-gray-800 dark:text-gray-200 max-h-[55vh] overflow-y-auto overscroll-contain"
        initial={{ opacity: 0 }}
