...
export default function SettingsDrawer({ open, onClose, onSaved }: Props) {
  ...
  const [tavilyWave, setTavilyWave] = useState(0);

  useEffect(() => {
    if (open) {
      const existing = localStorage.getItem("openrouter_key") || "";
      setKey(existing);
      const t = localStorage.getItem("tavily_key") || "";
      setTavily(t);
    }
  }, [open]);
  ...
  function saveTavily() {
    localStorage.setItem("tavily_key", tavily.trim());
    setTavilyWave((n) => n + 1);
    onSaved?.();
    setTimeout(onClose, 350);
  }
      ...
      {open && (
        <div className="fixed inset-0 z-50">
          ...
          <motion.aside
            ...
            className="absolute right-0 top-0 h-full w-full max-w-md bg-[#0B0E1A] text-white shadow-xl border-l border-white/10"
          >
            ...
            <div className="p-5 space-y-6">
              ...
              <div className="relative group flex items-center gap-3 rounded-[26px] border border-white/10 bg-white/5 backdrop-blur-2xl px-4 py-3 overflow-hidden">
                {/* Static aura + smooth fade like primary input */}
                <div
                  aria-hidden
                  ...
                  style={{
                    background:
                      "conic-gradient(from 180deg at 50% 50%, rgba(99,102,241,0.32), rgba(16,185,129,0.25), rgba(244,114,182,0.22), rgba(99,102,241,0.32))",
                    filter: "blur(8px)",
                  }}
                />
                {/* inner ring */}
                ...
                <input
                  ...
                  className="relative z-10 flex-1 bg-transparent outline-none placeholder:text-gray-400/80 text-gray-100"
                />
                <button
                  onClick={save}
              ...
              <div className="space-y-4 pt-2">
                <label className="text-sm text-white/70 inline-flex items-center gap-2"><Globe className="h-4 w-4"/> Tavily API Key (optional, enables Web Search)</label>
                <div className="relative group flex items-center gap-3 rounded-[26px] border border-white/10 bg-white/5 backdrop-blur-2xl px-4 py-3 overflow-hidden">
                  <div
                    aria-hidden
                    className="pointer-events-none absolute -inset-[1.5px] rounded-[28px] opacity-0 group-focus-within:opacity-100 group-hover:opacity-100 transition-opacity duration-300 ease-out"
                  ...
                  />
                  <div className="pointer-events-none absolute inset-0 rounded-[26px] ring-0 group-focus-within:ring-4 ring-white/10 transition-all duration-500" />
                  <button type="button" className="relative z-10 grid place-items-center h-10 w-10 rounded-xl text-white group/icon">
                    <span aria-hidden className="absolute inset-0 rounded-xl bg-white/10 opacity-0 scale-75 transition-all duration-300 group-hover/icon:opacity-100 group-hover/icon:scale-100" />
                ...
                <p className="text-xs text-white/50">Docs: Tavily Search basic, max 10 results. Key is stored locally.</p>
              </div>
            </div>
          </motion.aside>
        </div>
